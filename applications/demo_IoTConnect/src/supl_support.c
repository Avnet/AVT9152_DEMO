...
int open_supl_socket(void)
{
	...
	proto = IPPROTO_TCP;
	port = htons(CONFIG_DEMO_SUPL_SERVER_PORT);
...
ssize_t supl_send(const void *p_buff, size_t nbytes, void *user_data)
{
	ARG_UNUSED(user_data);
	return send(supl_fd, p_buff, nbytes, 0);
}

int supl_log(int level, const char *fmt, ...)
{
	char buffer[256] = { 0 };
	va_list args;
...
ssize_t supl_recv(void *p_buff, size_t nbytes, void *user_data)
{
	ARG_UNUSED(user_data);
