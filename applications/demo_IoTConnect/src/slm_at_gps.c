...
LOG_MODULE_REGISTER(gps, CONFIG_DEMO_IOTCONNECT_LOG_LEVEL);
...
extern struct k_work_q application_work_q;
...
int connect_supl_server(void)
{
	...
	supl_fd = ret;
	ret = getaddrinfo(CONFIG_DEMO_SUPL_SERVER, NULL, &hints, &res);
	if (ret || res == NULL) {
	...
	server.sin_port = htons(CONFIG_DEMO_SUPL_SERVER_PORT);
...
static void gps_thread_fn(void *arg1, void *arg2, void *arg3)
{
	...
	while (true) {
		...
		case NRF_GNSS_AGPS_DATA_ID:
			...
			k_work_submit_to_queue(&application_work_q, &supl_work);
#endif
			break;
